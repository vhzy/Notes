...
# 30.基于CLIP/ViT模型搭建相似图像检索系统

这次我们依然使用上次的水果蔬菜数据集，分别使用CLIP的ViT/B32和resnet(resnet50/150均可)

来提取视觉特征，然后使用这两个模型提取的特征进行余弦相似度计算

最后，给出query图片的相似度图片以及它们的相似度分数

clip的安装方式可见openai的官方代码

使用CLIP提取特征大概需要十几分钟
resnet系列提取的train特征都是26M，clip模型提取出来7M的特征表示

提取特征命令和相似检索的命令：

```python
python /dataset_fruit_veg/image_search_engine.py --model_name=clip
python /dataset_fruit_veg/image_search_engine.py --model_name=clip --mode = predict

python /dataset_fruit_veg/image_search_engine.py --model_name=resnet150
python /dataset_fruit_veg/image_search_engine.py --model_name=resnet150 --mode = predict
```
CLIP结果展示：
![卡通香蕉](https://pic4.zhimg.com/80/v2-8c2574f290529c0b5606e71a41735a13.png)

![带文字的香蕉](https://pic4.zhimg.com/80/v2-38bafd4fd7efb93226dd1d7477ce400c.png)

注意，如果使用resnet，第二张图是无法取得这种效果的
这得益于resnet的视觉语言对比学习
