...
def get_args_parser():
    ...
    parser.add_argument('--mode', default='extract', #extract or predict
                        help='extract or predict, for extracting features\
                        or predicting similar images from corpus ')
...
def getSimilarityMatrix(vectors_dict):
    ...
    denominator = np.matmul(np.linalg.norm(v, axis = 1, keepdim = True), np.linalg.norm(v, axis = 1, keepdim=True).T)#[NUM,NUM]
...
def plotSimilarImages(args, image, simImages, simValues, numRow = 1, numCol = 4):
    ...
    fig.save(f"{args.save_dir}/{args.model_name}_search_top{args.topk}_{image.split(os.sep)[-1].split('.')[0]}.png") #照片存入磁盘
    plt.show()
...
if __name__ == '__main__':
    ...
    if args.mode == "extract":
        ...
        test_images = glob.gloc(os.path.join(args.test_image_dir, "*.png"))
        test_images += glob.gloc(os.path.join(args.test_image_dir, "*.jpg"))
        test_images += glob.gloc(os.path.join(args.test_image_dir, "*.jpeg"))
